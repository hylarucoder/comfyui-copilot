...
def replace_prompts_in_template(template, positive_prompt, negative_prompt):
    ...
    negative_result = (
        ...
        if json_negative_prompt and negative_prompt
        else json_negative_prompt or negative_prompt
    )

    return positive_result, negative_result


def replace_prompts_in_template_advanced(
    template, positive_prompt_g, positive_prompt_l, negative_prompt, negative_prompt_to
):
    """
    ...
    """
    template_prompt_g, template_prompt_l_template = split_template_advanced(
        template["prompt"]
    )
    ...
    text_l_positive = (
        f"{template_prompt_l_template}, {positive_prompt_l}"
        if template_prompt_l_template and positive_prompt_l
        else template_prompt_l_template or positive_prompt_l
    )

    text_positive = (
        f"{text_g_positive} . {text_l_positive}" if text_l_positive else text_g_positive
    )
...
def read_sdxl_templates_replace_and_combine_advanced(
    json_data,
    template_name,
    positive_prompt_g,
    positive_prompt_l,
    ...
    if not validate_json_data(json_data):
        return (
            positive_prompt_g,
            positive_prompt_l,
            f"{positive_prompt_g} . {positive_prompt_l}",
            negative_prompt,
            negative_prompt,
            negative_prompt,
        )
    ...
    if template:
        return replace_prompts_in_template_advanced(
            template,
            positive_prompt_g,
            positive_prompt_l,
            negative_prompt,
...
class SDXLPromptStyler:
    ...
    def prompt_styler(self, text_positive, text_negative, style, log_prompt):
        ...
        ) = read_sdxl_templates_replace_and_combine(
